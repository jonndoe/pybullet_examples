...
objectUid = p.loadURDF(os.path.join(urdfRootPath, "random_urdfs/000/000.urdf"), basePosition=[0.73,0,0.1])
p.setGravity(0,0,-10)
p.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=0, cameraPitch=-40, cameraTargetPosition=[0.55,-0.35,0.2])


position = p.addUserDebugParameter('Join 2 position', 0, 5)

object_x_position = p.addUserDebugParameter('Object x pos', 0, 1)
...
while True:
    ...
    user_join2 = p.readUserDebugParameter(position)
    user_object_x_pos = p.readUserDebugParameter(object_x_position)

    p.setJointMotorControl2(pandaUid, 2, 
                        p.POSITION_CONTROL, targetPosition=user_join2)
    ...
    if current_state == 1:
        p.setJointMotorControl2(pandaUid, 1, 
                        p.POSITION_CONTROL,math.pi/4.+.19)
        p.setJointMotorControl2(pandaUid, 3, 
